...
for claim in tqdm(Claims, desc="Generating summaries", unit="abstract"):
    inputs = tokenizer.encode("summarize: " + claim, return_tensors="pt").to(device)
    outputs = model.generate(inputs, max_length=150, num_beams=4, early_stopping=True)
    generated_text = tokenizer.decode(outputs[0], skip_special_tokens=True)
    generated_texts.append(generated_text)

# Add the generated text to the DataFrame
df_subset['Claims_Summary_t5_base'] = generated_texts

# Save the modified DataFrame to a new Excel file
df_subset.to_excel('Claims_Summary_t5_base.xlsx', index=False)
